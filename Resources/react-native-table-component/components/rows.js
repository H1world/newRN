...
class Row extends Component {
  static propTypes = {
    ...
    textStyleY: Text.propTypes.style,
  }

  render() {
    const { data, style, widthArr, height, flexArr, textStyle, textStyleY, borderStyle, ...props} = this.props;
    let widthNum = 0;
    if (widthArr) {
      ...
      <View style={[
      ...
      ]}>
        {
          data.map((item, i) => {
            ...
            return <Cell key={i} data={item} width={width} height={height} flex={flex} textStyle={textStyle} textStyleY={item > 0 ? textStyleY : null} borderStyle={borderStyle} {...props}/>
          })
        }
      </View>
      : null
